...
function BoxPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // If this box was generated by the AI and has an id on the server, call save endpoint
      if (aiBox && (aiBox as any).id) {
        const res = await fetch(`${API_URL}/api/packs/${(aiBox as any).id}/save`, {
          method: "POST",
          headers: {
        ...
        if (!res.ok) throw new Error("Failed to save StudyBox on server");
      ...
      if (token && aiBox) {
        ...
        if (!res.ok) {
          throw new Error("Server rejected create pack request");
        }
      ...
      // Fallback to localStorage if server not available or token missing
      saveBox({
      ...
      toast.success("StudyBox saved locally (offline)");
    } catch (err) {
      console.error(err);
